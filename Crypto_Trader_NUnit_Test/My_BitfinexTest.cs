...
{
    /// <summary>
    ///     Unit Test Class for My_Bitfinex module from ExchangesModels
    /// </summary>
    [TestFixture()]
    public class My_BitfinexTest
    {
        My_Bitfinex bitfinex;
        int numTest = 1;
        double marginError = 0.05;

        [SetUp()]
        public void My_BitfinexTest_Init()
        {
            Console.WriteLine("------ My_Bitfinex Unit Test N° " + numTest + "------");
        ...
        public void GetPrice_Test()
        {
            ...
            {
                onePrice = this.bitfinex.GetPrice(crypto);
        ...
        {
            Price onePrice;
            onePrice = this.bitfinex.GetPrice("fake price");
        ...
        public void GetFolio_Test()
        {
            ...
            dicoFolio = this.bitfinex.GetFolio();
        ...
        public void GetAdress_Test()
        {
            ...
            foreach (string crypto in Enum.GetNames(typeof(MainCryptos)))
            {
                ...
                {
                    adress = this.bitfinex.GetAdress("BCC");
                }
                else
                {
                    adress = this.bitfinex.GetAdress(crypto);
                }
        ...
        public void GetAdress_TestFake()
        {
            ...
            adress = this.bitfinex.GetAdress("fake crypto");
        ...
        public void Buy_test()
        {
            ...
            foreach (string crypto in Enum.GetNames(typeof(MainCryptos)))
            {
                spotPrice = this.bitfinex.GetPrice(crypto).price;
                ...
                if (crypto == MainCryptos.BCH.ToString())
                {
                    (sucess, message) = this.bitfinex.Buy("BCC", amount, spotPrice);
                }
                else
                {
                    (sucess, message) = this.bitfinex.Buy(crypto, amount, spotPrice);
                }
        ...
        public void Sell_test()
        {
            ...
            foreach (string crypto in Enum.GetNames(typeof(MainCryptos)))
            {
                ...
                if (crypto == MainCryptos.BCH.ToString())
                {
                    (sucess, message) = this.bitfinex.Sell("BCC", amount, spotPrice);
                }
                else
                {
                    (sucess, message) = this.bitfinex.Sell(crypto, amount, spotPrice);
                }
        ...
        public void GetAccountInfos_Test()
        {
            ...
            accountInfos = this.bitfinex.GetAccountInfos();
        ...
        public void CancelOrder_Test()
        {
            ...
            foreach (string crypto in Enum.GetNames(typeof(MainCryptos)))
            {
                sucess = this.bitfinex.CancelOrder(crypto, orderId);
        ...
        public void GetOrderId_Test()
        {
            ...
            orderId = this.bitfinex.GetOrderId(ccy);
        ...
        public void GetOpenOrders_Test()
        {
            ...
            foreach (string crypto in Enum.GetNames(typeof(MainCryptos)))
            {
                if (crypto == MainCryptos.BCH.ToString())
                {
                    openOrders = this.bitfinex.GetOpenOrders("BCC");
                }
                else if (crypto != MainCryptos.EOS.ToString() && crypto != MainCryptos.IOTA.ToString() &&
                    crypto != MainCryptos.ADA.ToString() && crypto != MainCryptos.USDT.ToString() &&
                    crypto != MainCryptos.TRX.ToString())
                {
                    openOrders = this.bitfinex.GetOpenOrders(crypto);
        ...
        public void SendCryptos_Test()
        {
            ...
            foreach (string crypto in Enum.GetNames(typeof(MainCryptos)))
            {
                ...
                if (crypto == MainCryptos.BCH.ToString())
                {
                    (sucess, message) = this.bitfinex.SendCryptos(adress, "BCC", amount);
                }
                else
                {
                    (sucess, message) = this.bitfinex.SendCryptos(adress, crypto, amount);
                }
